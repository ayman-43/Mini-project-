    ...
    <div className="flex flex-col h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <div className={`flex-1 ${messages.length > 0 && "overflow-y-auto scrollbar-custom"} px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8 space-y-3 sm:space-y-4 md:space-y-6 scroll-smooth`}>
        ...
        {messages.map((message) => (
          <div
            key={message.id}
            className={`flex gap-2 sm:gap-3 animate-slide-up ${message.role === 'user' ? 'justify-end' : 'justify-start'
              }`}
          >
            {message.role === 'assistant' && (
              <div className="flex-shrink-0">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/20 ring-2 ring-white dark:ring-gray-800">
            ...
            <div
              className={`group relative max-w-[90%] sm:max-w-[85%] md:max-w-[75%] lg:max-w-[70%] ${message.role === 'user'
                ...
                } px-4 py-3 sm:px-5 sm:py-4 transition-all duration-300 hover:scale-[1.01]`}
            >
              {message.role === 'assistant' ? (
                ...
                <p className="text-xs sm:text-sm md:text-[15px] leading-relaxed whitespace-pre-wrap break-words font-medium">
                  {message.content}
                </p>
              )}
            </div>
            ...
            {message.role === 'user' && (
              <div className="flex-shrink-0">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600 flex items-center justify-center shadow-lg ring-2 ring-white dark:ring-gray-800">
                  <span className="text-xs sm:text-sm font-bold text-gray-600 dark:text-gray-300">You</span>
                </div>
              </div>
            )}
          </div>
        ...
        {loading && (
          <div className="flex gap-2 sm:gap-3 animate-slide-up justify-start">
            <div className="flex-shrink-0">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/20 ring-2 ring-white dark:ring-gray-800">
                <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
              </div>
            </div>
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md shadow-xl shadow-gray-200/50 dark:shadow-gray-900/50 rounded-2xl rounded-tl-sm border border-white/20 dark:border-gray-700/30 px-4 py-3 sm:px-5 sm:py-4 flex items-center gap-1.5">
        ...
        <form onSubmit={handleAnalyze} className="max-w-6xl mx-auto space-y-2 sm:space-y-3">
          ...
          <div className="relative flex items-end gap-1.5 sm:gap-2 bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-2xl shadow-blue-500/10 dark:shadow-blue-500/5 p-1.5 sm:p-2 border-2 border-gray-200 dark:border-gray-700 focus-within:border-blue-500 dark:focus-within:border-blue-400 focus-within:shadow-blue-500/20 transition-all duration-300">
              ...
              >
                ...
                {filteredSymptoms.map((symptom, index) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm transition-colors duration-150 ${index === selectedIndex
            ...
            <button
              ...
              disabled={loading}
              className={`flex-shrink-0 p-2 sm:p-2.5 md:p-3 rounded-lg sm:rounded-xl transition-all duration-300 ${isListening
